...
### Part 2: Open Cloud Shell Editor
...
### Part 4: Configure Project & Enable APIs
...
```bash
cd ~
git clone https://github.com/anthropic-ai/gemini-motion-lab.git
cd gemini-motion-lab
```
...
│   ├── public/                  # Static assets & showcase videos
│   ├── Dockerfile
│   └── nginx.conf
...
> **What `setup.sh` does:**
>
> 1. Creates a **GCS bucket** (`gemini-motion-lab-$PROJECT_ID`) to store uploaded videos, frames, and generated outputs
> 2. Creates a **service account** (`gemini-motion-lab-sa`) for the backend to access GCS and Vertex AI
> 3. Generates a **`.env` file** with all required environment variables pre-filled

### 2. Grant IAM Roles

Now grant the required permissions to the service account. Run each command below — we'll explain what each role does.

👉💻 First, set your project ID variable:

```bash
export PROJECT_ID=$(cat ~/project_id.txt)
```

👉💻 Grant **Storage Admin** — allows the backend to upload/download videos and frames:
...
> aside positive
> **`roles/storage.admin`** gives the service account full access to read, write, and manage objects in Cloud Storage. The backend needs this to upload recorded videos, store generated frames, and serve composed videos.

👉💻 Grant **Vertex AI User** — allows the backend to call Gemini and Veo:
...
> aside positive
> **`roles/aiplatform.user`** allows the service account to make predictions using Vertex AI models. This is required for Gemini video analysis, Nano Banana avatar generation, and Veo video creation.

👉💻 Grant **Service Account Token Creator** — allows the backend to generate signed URLs for GCS:
...
> aside positive
> **Why signed URLs?**
> When the share page needs to serve a video to a user's mobile phone, the backend generates a **time-limited signed URL** for the GCS object. This requires the `serviceAccountTokenCreator` role so the Cloud Run service (which runs as the compute service account) can sign tokens on behalf of the motion lab service account.
...
👉💻 Deploy the frontend, pointing it to your backend URL:

```bash
cd ~/gemini-motion-lab/frontend
